...
CREATE TABLE IF NOT EXISTS DATAWAREHOUSE.LEADS_DISTRIBUTION.TSK_CAMPAIGN_AUTOMATION_CONFIG (
  ...
  UPDATE_HLL_PROCEDURE   VARCHAR,                     -- proc (CALL proc(campaignid)) that updates the HLL; chosen from TSK_HLL_UPDATE_PROCEDURES
...
ALTER TABLE DATAWAREHOUSE.LEADS_DISTRIBUTION.TSK_CAMPAIGN_AUTOMATION_CONFIG
  ADD COLUMN IF NOT EXISTS UPDATE_HLL_PROCEDURE VARCHAR;
